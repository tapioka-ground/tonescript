...
| `MIX` の鍵 | 意味 |
|---|---|
| `bus` | 送り先のバスの名前（`BUSES` に書いたもの）。書かなければ全体へ直接（下に） |
...
**大きいところだけを小さくする。** 大小の差が縮むので、全体を持ち上げても
割れなくなり、他に埋もれずに前へ出てくる。

歌に一番効く。人は一定の音量で歌えないので、生のままだと大きい所が割れて
小さい所が聞こえない。
...
| 鍵 | 中身 | 既定 |
|---|---|---|
| `threshold` | ここを越えたぶんを押さえる（dB）。-60〜0 | -18 |
| `ratio` | 4 なら「4dB 越えたら 1dB だけ出す」。1 で何もしない | `comp` を書けば 4 |
| `attack` | 効き始めるまで（ミリ秒） | 10 |
| `release` | 戻るまで（ミリ秒） | 120 |
| `knee` | 曲がり角の丸さ（dB） | 6 |
| `makeup` | 押さえたぶんを持ち上げる（dB） | 0 |

**聞いて分かるのはアタック。** 遅く（30〜80ms）すると打撃の頭が通り抜けて
パンチが残る。速く（1ms 未満）すると頭まで捕まえて平らになる。

**EQ の次に通る。** 整えてから押さえる。

### `BUSES` — まとめて1本のフェーダーで扱う

太鼓は1つではない。キック・スネア・ハット・パーカッションと分かれていて、
その中の釣り合いを取るのに時間が掛かる。そのあとで「太鼓が全体に対して
大きい」と気付いたとき、フェーダーを1本ずつ下げていくと、せっかく取った
釣り合いが崩れる。

バスへまとめて送っておけば、1本動かすだけで済む。中の釣り合いはそのまま。

```rhai
let BUSES = #{
    drumbus: #{ label: "太鼓", gain: 0.9, comp: #{ threshold: -14, ratio: 3 } },
...
| `BUSES` の鍵 | 意味 | 既定 |
|---|---|---|
| `label` | 画面に出す名前 | バスの名前そのもの |
| `gain` | まとめての音量。0〜8 | 1.0 |
| `pan` | 左右の位置。-1 が左、1 が右 | 0 |
| `eq` | パートと同じ3band | 素通し |
| `comp` | パートと同じ押さえ込み | 切 |
| `reverb` | バスまとめて残響へ送る | 0 |
| `duck` | サイドチェインの掛かり具合 | 0 |

掛かる道具も順番もパートと同じ。EQ → 押さえ込み → 凹み → 残響へ送る →
左右 → バスの音量、の順で通って全体へ出る。

2つだけ覚えておくこと。

- **バスの押さえ込みは、パートごとに掛けるのとは別物。** 足し合わさった
  音を見るので、スネアが強く鳴ると太鼓全部が一緒に沈む。これが「1つの
  音として馴染む」理由であり、やり過ぎると全体が波打つ理由でもある。
- **バスからバスへは送れない。** パートはバスへ、バスは全体へ。入れ子に
  すると、輪になって止まらない繋ぎ方が書けてしまう。

`BUSES` に無い名前を書いたら、曲を読んだところで止める。黙って全体へ
流すと「送ったのにフェーダーが効かない」という分かりにくい形で出る。
